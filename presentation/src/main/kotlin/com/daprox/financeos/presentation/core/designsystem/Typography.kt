...
// Google Fonts provider — Inter is fetched at runtime via Google Play Services.
// The certificate array (R.array.com_google_android_gms_fonts_certs) is declared
// in presentation/src/main/res/values/font_certs.xml.
...
// Four weights cover the full type scale — Normal (body), Medium (labels/titles),
// SemiBold (headlines), Bold (display).
...
// Full Material3 type scale using Inter for every role.
// Line heights follow the M3 spec; letter spacing is tightened for display roles
// where Inter's default tracking is slightly wide.
internal val FinanceOSTypography = Typography(
    displayLarge  = TextStyle(fontFamily = InterFontFamily, fontWeight = FontWeight.Bold,     fontSize = 57.sp, lineHeight = 64.sp, letterSpacing = (-0.25).sp),
    displayMedium = TextStyle(fontFamily = InterFontFamily, fontWeight = FontWeight.Bold,     fontSize = 45.sp, lineHeight = 52.sp, letterSpacing = 0.sp),
    displaySmall  = TextStyle(fontFamily = InterFontFamily, fontWeight = FontWeight.Bold,     fontSize = 36.sp, lineHeight = 44.sp, letterSpacing = 0.sp),
